# -*- coding: utf-8 -*-
"""
Created on Sun Jun 26 00:35:30 2022

@author: Hello
"""

#期末作业T4
#根据给定的两幅DEM数据，通过Python代码从中获取一幅新的DEM数据，
#新DEM数据的范围是：左下角(118.2, 31.9) 右上角(118.4, 32.1)。
#针对新DEM数据，编写算法提取高程数据中所有的局部最高点，所谓局部最高点就是高程大于周围所有栅格的栅格所在的位置。
#使用matplotlib显示DEM数据，并在局部最高点放置旗子图片（可用flag-x.png）。
...
if __name__ == "__main__":
    ...
    # 读取ASTER GDEM遥感影像
    demgrid, proj, geotrans, row, column, band, type = read_img("E:\派森\期末\T4\clip1.tifa.tif")
    oridata = demgrid
    # 为计算梯度给影像添加周围一圈数据
    ...
    slopeT = write_img(r"E:\派森\期末\T4\slope11.tif", tar_proj, geotrans, slope, type)
    aspectT = write_img(r"E:\派森\期末\T4\aspect11.tif", tar_proj, geotrans, aspect, type)
